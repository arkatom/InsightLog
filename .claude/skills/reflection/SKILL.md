...
2. **ディレクトリ確認・作成**: `.claude/memory/reflection/` ディレクトリが存在しない場合は作成
3. **ファイル出力**: `.claude/memory/reflection/YYYYMMDD_{やったことを簡潔に}.md` にWrite
4. **完了報告**: 保存したファイルパスを報告
5. **heartbeat**: !`claude /heartbeat` を実行して改善サイクルに反映
